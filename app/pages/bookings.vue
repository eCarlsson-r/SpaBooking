...
const confirmCancel = async (session) => {
  ...
  const confirm = window.confirm(t('bookings.cancelConfirm'))
  if(confirm) {
...
const openReschedule = (session) => {
  useRescheduleStore().open(session)
}
</script>

<template>
  <div class="pt-20 pb-32 px-6">
    <h1 class="text-3xl font-black text-primary-900 mb-8">{{ t('bookings.title') }}</h1>
    ...
    <div v-else class="text-center py-20">
      <p class="text-slate-400 font-medium">{{ t('bookings.noSessions') }}</p>
