...
<template>
  <div class="space-y-6">
    <div class="text-center">
      ...
      <h2 class="text-lg font-bold">{{ t('confirm.orderSaved') }}</h2>
      <p class="text-xs text-slate-500">{{ t('confirm.orderSavedSubtitle') }}</p>
    </div>

    <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-3">
      <div class="flex justify-between items-center">
        <span class="text-xs text-slate-500 uppercase">{{ t('confirm.treatment') }}</span>
        <span class="font-bold text-sm">{{ selection.treatment?.name }}</span>
      </div>
      <div class="flex justify-between items-center">
        <span class="text-xs text-slate-500 uppercase">{{ t('confirm.time') }}</span>
        <span class="font-bold text-sm">{{ selection.session_date }} @ {{ selection.session_time }}</span>
      </div>
      <div class="flex justify-between items-center">
        <span class="text-xs text-slate-500 uppercase">{{ t('confirm.therapistRoom') }}</span>
        <span class="font-bold text-sm">{{ selection.employee?.name }} ({{ selection.room?.name }})</span>
      </div>
      <div class="border-t pt-3 flex justify-between items-center">
        <span class="font-bold">{{ t('confirm.total') }}</span>
