  ...
  <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 backdrop-blur-sm">
    <div class="bg-white w-full max-w-md rounded-t-[3rem] p-8 animate-slide-up">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-black italic text-[#8B6E1C]">{{ t('reschedule.title') }}</h2>
        <button class="text-slate-300" @click="$emit('close')">✕</button>
      </div>

      <div class="mb-6">
        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-3">{{ t('reschedule.selectNewDate') }}</p>
      ...
      <div class="mb-8">
        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-3">{{ t('reschedule.selectNewTime') }}</p>
...
const updateAppointment = async () => {
  ...
  try {
    ...
    alert(t('reschedule.failedToReschedule'))
  }
}
</script>